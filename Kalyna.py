...
S_BLOCK = [[168, 67, 95, 6, 107, 117, 108, 89, 113, 223, 135, 149, 23, 240, 216, 9,
...
INV_S_BLOCK = [[164, 162, 169, 197, 78, 201, 3, 217, 126, 15, 210, 173, 231, 211, 39, 91,
...
VECTOR = [0x01, 0x01, 0x05, 0x01, 0x08, 0x06, 0x07, 0x04]
INV_VECTOR = [0xAD, 0x95, 0x76, 0xA8, 0x2F, 0x49, 0xD7, 0xCA]
...
# Саб байтс
def sbox(x):
    return [S_BLOCK[i % 4][x[i]] for i in range(len(x))]


# Инверсивный саб байст
def sbox_inv(x):
    return [INV_S_BLOCK[i % 4][x[i]] for i in range(len(x))]
...
def scalar_mult(x, y):
    ...
    for i in range(len(x)):
        res  ^= galoisMult(x[i],y[i])
    return res

# Сдвиг вектора вправо
...
# Сдвиг вектора влево
def leftshiftvector(x, i):
    l = len(x)
    i = i % l
    return x[i:] + x[:i]

# Мерзотный миксколумн
def mcol(x):
    res = []
    for i in range(2):
        for j in range(8):
            res.append(scalar_mult(x[i * 8:i * 8 + 8], rightshiftvector(VECTOR, j)))
    return res


# Инверсивно-мерзотный миксколумн
def mcol_inv(x):
    res = []
    for i in range(2):
        for j in range(8):
            res.append(scalar_mult(x[i * 8:i * 8 + 8], rightshiftvector(INV_VECTOR, j)))
    return res


# Переводим 64 бит как список байтов
def int2list(x):
    return [x >> i & 0xff for i in [56, 48, 40, 32, 24, 16, 8, 0]]


# Переводим список байтов как 64 бита
def list2int(x):
...
# Представляет ключ и внутреннее состояние как пару 64-битных чисел и возвращает его сумму по модулю 2 ^ 64
def key_add(k, state):
    k0, k1 = list2int(list(reversed(k[:8]))), list2int(list(reversed(k[8:])))
...
# Представляет ключ и внутреннее состояние как пару 64-битных чисел и возвращает его вычитание по модулю 2 ^ 64
def key_sub(k, state):
    k0, k1 = list2int(list(reversed(k[:8]))), list2int(list(reversed(k[8:])))
    state0, state1 = list2int(list(reversed(state[:8]))), list2int(list(reversed(state[8:])))
...
# 128-битное расширение(развертывания) ключа для 128-битного блочного шифрования / дешифрования
def keyexpansion(key):
    k0, k1 = key[:], key[:]
    state = [0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]
    state = key_add(k0, state)
    state = sbox(state)
    state = srow(state)
    state = mcol(state)
    state = xor(k0, state)
    ...
    state = sbox(state)
    state = srow(state)
    intermediatekey = mcol(state)
    even_round_keys(key, intermediatekey)
    return roundkeys

# FML Генерируем ключ
def even_round_keys(key, intermediatekey):
    roundkeys.clear()
    tmv = [0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00]
    id = key[:]
    state = key[:]
    for i in range(0, 11, 2):
        kt_round = key_add(intermediatekey, [k << i // 2 for k in tmv])
        state = key_add(rightshiftvector(id, 4 * i), kt_round)
        state = sbox(state)
        state = srow(state)
        state = mcol(state)
        state = xor(state, kt_round)
        state = sbox(state)
        state = srow(state)
        state = mcol(state)
        state = key_add(state, kt_round)
        roundkeys.append(state)
        if i < 10:
            roundkeys.append(leftshiftvector(state, 7))

# Шифруем наш текст
def encryption(m, key):
    roundkeys = keyexpansion(key)
    state = key_add(roundkeys[0], m)
    for i in range(1, 10):
        state = sbox(state)
        state = srow(state)
        state = mcol(state)
    ...
    state = srow(state)
    state = mcol(state)
    state = key_add(roundkeys[10], state)
    return state

# Дешифруем наш текст
def decryption(c,key):
    roundkeys = keyexpansion(key)
    state = key_sub(roundkeys[10],c)
    state = mcol_inv(state)
    state = srow(state)
    ...
    for i in range(9, 0, -1):
        ...
        state = srow(state)
        state = sbox_inv(state)
    state = key_sub(roundkeys[0],state)
    return state
...
def save(event):
    ...
    with io.open(path1, 'w', encoding="utf-8") as f:
        f.write(b.get(1.0, END))

def Encrypt(event):
    key = log.get()
    txt = a.get(1.0, END).rstrip("\n").lower()

    if len(txt) != 32:
        print('Наш блок должень иметь длину 128 бит !!!')
        return
    elif len(key) != 32:
        print('Ключ тоже долже быть 128 бит !!!')
        return

    key = list(binascii.unhexlify(key))
    txt = list(binascii.unhexlify(txt))

    res = encryption(txt, key)
...
im=Button(text='Импортировтаь из файла')
im.bind('<Button-1>',Important)
im.pack(fill = X)

path = Label(root)
path['text'] = 'Входной текст'
path.pack()

a = Text(root, bg='black', fg='white', height = 10)
a.pack(fill=X)
...
log = Entry(root, width=30)
log.pack(fill=X)

en=Button(text='Шифровать')
en.bind('<Button-1>',Encrypt)
en.pack(fill = X)

de=Button(text='Дешифровать')
de.bind('<Button-1>',Decrypt)
de.pack(fill = X)

sa=Button(text='Сохранить в файл')
sa.bind('<Button-1>',save)
sa.pack(fill = X)

ex = Label(root)
ex['text'] = 'Выходной текст'
ex.pack()

b = Text(root, bg='black', fg='white',height = 10)
b.pack(fill=X)
